...
export class ListLearningSettingsController implements IController {
  ...
  constructor(private readonly listLearningSettingsByUser: ListLearningSettingsByUser) { }
  ...
  async handle(request: IRequest<unknown>): Promise<IResponse<unknown>> {
    ...
    const listSettings = await this.listLearningSettingsByUser.execute(body.userId)
    return {
      body: listSettings,
      statusCode: 200
    }
