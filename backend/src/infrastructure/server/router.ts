...
router.post("/", async (request, response) => {
  ...
  const rabbitMQBroker = RabbitMQBrokerAdvanced.getInstance(
    "amqp://localhost:5672",
  );
  await rabbitMQBroker.init();
  const handleVoiceInputUseCase = new HandleVoiceInputUseCase(
    evolutionMediaMessageService,
    rabbitMQBroker,
  );
  const whatsAppAdapter = new WhatsAppAdapter();

  const appConversationOrchestratorUseCase =
  ...
  const evolutionController = new EvolutionController(
    appConversationOrchestratorUseCase,
  );
